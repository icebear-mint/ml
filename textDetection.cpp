...
vector<pair<CvPoint,CvPoint> > findBoundingBoxes( vector<vector<Point2d>> & components,vector<Chain> & chains,vector<pair<Point2d,Point2d> > & compBB,IplImage * output)
    {
    vector<pair<CvPoint,CvPoint> > bb;
    bb.reserve(chains.size());
    for (vector<Chain>::iterator chainit = chains.begin(); chainit != chains.end(); chainit++) {
...
std::vector<std::pair<CvPoint,CvPoint> > findBoundingBoxes( std::vector<std::vector<Point2d> > & components, IplImage * output) {
                ...
                while (true) {
                    ...
                    if ((int)(floor(curX)) != curPixX || (int)(floor(curY)) != curPixY) {
                        ...
                        if (CV_IMAGE_ELEM ( edgeImage, uchar, curPixY, curPixX) > 0) {
                            ...
                            if (acos(G_x * -G_xt + G_y * -G_yt) < PI/2.0 ) {//判断是否在一条直线上
                                ...
                                for (std::vector<Point2d>::iterator pit = points.begin(); pit != points.end(); pit++) {
                                    if (CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) < 0) {
                                        ...
                                        CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x) = std::min(length, CV_IMAGE_ELEM(SWTImage, float, pit->y, pit->x));//为中间的像素点赋值
                                    }
                                }
                                r.points = points;
                                rays.push_back(r);//返回未被废弃的射线rays
                            }
                            break;
                        }
                    }
...
//综合获得连通域sw的信息，均值mean，方差variance，中值median等
